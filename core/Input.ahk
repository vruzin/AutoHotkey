...
class PuntoInput {
    ...
    static IsWordChar(ch) {
        ...
        if (ch = "-" || ch = "'")
            return true
        ; Клавиши, которые в RU-раскладке являются буквами, а в EN —
        ; знаками. Если пользователь набирает русское слово, оставаясь
        ; в EN-раскладке (типа cby[hjafpfnhjy = синхрофазатрон),
        ; эти символы должны считаться частью слова, иначе буфер
        ; обрывается на [ и Pause переводит только часть.
        ;   [ → х     ] → ъ     ; → ж     ' → э
        if (ch = "[" || ch = "]" || ch = ";" || ch = "`"")
            return true
        return false
